...
| `mouse_capture` | 捕获鼠标事件。`true`：jcode 拦截鼠标用于 TUI 内部操作（点击切换焦点、滚轮滚动、点击复制徽章等），终端原生的选中即复制/右键粘贴被屏蔽；`false`：鼠标事件穿透到终端，终端原生选中复制、右键粘贴可用，但 TUI 内部鼠标交互失效 | `false` | `true` |
...
| 键 | 说明 | 示例值 | 默认值 |
|-----|------|--------|--------|
| `engine` | 搜索引擎 | `"duckduckgo"` | `"duckduckgo"` |
| `fallback_engines` | 备用搜索引擎列表 | `["bing"]` | `[]` |
...
| 键 | 说明 | 示例值 | 默认值 |
|-----|------|--------|--------|
| `ntfy_server` | ntfy.sh 服务器地址 | `"https://ntfy.sh"` | `"https://ntfy.sh"` |
