...
| 键 | 示例值 | 默认值 | 说明 | 环境变量覆盖 |
|-----|------|--------|--------|----------|
| `memory` | `true` | `true` | 启用记忆系统 | `JCODE_MEMORY_ENABLED` |
| `swarm` | `true` | `true` | 启用群体协作 | `JCODE_SWARM_ENABLED` |
| `message_timestamps` | `true` | `true` | 显示消息时间戳 | `JCODE_MESSAGE_TIMESTAMPS` |
| `persist_memory_injections` | `false` | `false` | 持久化记忆注入 | `JCODE_PERSIST_MEMORY_INJECTIONS` |
| `kv_cache_miss_notices` | `true` | `true` | 缓存未命中通知 | `JCODE_KV_CACHE_MISS_NOTICES` |
| `update_channel` | `"stable"` | `"stable"` | 更新渠道（stable/release / main/nightly/edge） | `JCODE_UPDATE_CHANNEL` |

> 所有功能开关都可以通过对应的环境变量在运行时覆盖，支持值：`true`/`false`/`1`/`0`。`update_channel` 的环境变量接受 `stable`（或 `release`）、`main`（或 `nightly`/`edge`）。
...
| `model_picker_providers` | — | — | `/model` 命令中仅显示这些提供商的模型（见下方说明） |
...
筛选 `/model` 命令中显示的提供商列表，隐藏本地不可用的提供商。

```toml
[provider]
# 只显示这些提供商的模型（未设置或空 = 显示全部）
...
| 路由 API 方法 | OAuth 或兼容配置 | `"claude-oauth"`, `"openai-compatible:myprofile"` |
| OpenAI 兼容 profile ID | `provider add` 创建的自定义配置名 | `"myprofile"`, `"my-api"` |

> 当前会话正在使用的模型始终可见，不受此过滤影响。
...
WebSocket 网关服务，用于 **iOS / Web 客户端**远程连接到 Jcode 服务器。启用后，服务器会在指定端口上监听 WebSocket 连接，配对后可通过 `jcode pair` 生成的配对码让手机或浏览器客户端接入。

> 此功能默认关闭。启用后请注意安全：绑定到 `0.0.0.0` 时局域网内均可访问，建议仅在受信网络或配合 Tailscale/WireGuard 等 VPN 使用。

| 键 | 示例值 | 默认值 | 说明 |
|-----|------|--------|--------|
| `enabled` | `false` | `false` | 启用 WebSocket 网关 |
| `port` | `7643` | `7643` | 监听端口 |
| `bind_addr` | `"0.0.0.0"` | `"0.0.0.0"` | 绑定地址（`0.0.0.0`=所有网卡，`127.0.0.1`=仅本机） |
...
| `JCODE_MEMORY_ENABLED` | 记忆系统开关（true/false） |
| `JCODE_SWARM_ENABLED` | Swarm 群体协作开关（true/false） |
| `JCODE_MESSAGE_TIMESTAMPS` | 消息时间戳开关（true/false） |
| `JCODE_PERSIST_MEMORY_INJECTIONS` | 持久化记忆注入开关（true/false） |
| `JCODE_KV_CACHE_MISS_NOTICES` | KV 缓存未命中通知开关（true/false） |
| `JCODE_UPDATE_CHANNEL` | 更新渠道（stable/release / main/nightly/edge） |
